...
test('ticket category management', async ({ page }) => {
  ...
  await expect(page.getByTestId('heading-page-title')).toHaveText('Create Ticket Category');

  await page.getByTestId('textbox-name').fill('Test Ticket Category');
  await page.getByTestId('textbox-description').fill('Test Ticket Category Description');
  ...
  await expect(page.getByTestId('heading-page-title')).toHaveText('Edit Ticket Category');
  await expect(page.getByTestId('textbox-name')).toHaveValue('Test Ticket Category');
  ...
  await expect(page.getByTestId('table-ticket-categories')).toContainText('Test Ticket Category');

  // search non-existent ticket-category
  await page.getByTestId('textbox-search').fill('Non-existent Ticket Category');

  await page.waitForURL('/ticket-categories?search=Non-existent+Ticket+Category');
  ...
  await page.getByTestId('textbox-description').fill('Test Ticket Category Description Updated');

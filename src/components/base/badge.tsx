...
export function Badge({ children, color, variant = "solid" }: BadgeProps) {
  return (
    <span className={cn(badgeClass({ color, variant }))}>
      {variant === "outline" && (
        <span className={badgeIndicatorClass({ color })} />
      )}
      {children}
    </span>
  );
...
const badgeClass = cva(
  "rounded-full px-2 py-1 text-xs font-medium inline-flex",
  {
    variants: {
      variant: {
        solid: "",

...
export const actionHandlers = [
      ...
      const filteredActions = matchSorter(
        storedActions.filter((action) => {
          if (filters.channel_id) {
            const unparsedStoredActionChannels =
              localforage.getItem("action_channel");
  ...
  rest.post("/api/actions", async (req) => {
    try {
      ...
      const isActionExisted = storedActions.some(
        (action) => action.cta_label === data.cta_label
      );

      if (isActionExisted) {
        throw new UnprocessableEntityError(
          "Action with the same name is already registered"
        );
      }

      const newAction: Action = {
        id: nanoid(),
        cta_icon_type: data.cta_icon_type,
        cta_icon_value: data.cta_icon_value,
